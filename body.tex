...
There is no formal \gls{EVMS} required from the funding Agencies (the \gls{NSF} and the \gls{DOE} Office of Science) so activities are planned in detail on an annual basis and effort towards that schedule of activities is tracked using agile tools.
%this needs discussion!

The annual planning process starts by reviewing, revising, and adding major milestones to the next fiscal year plan.
These are centered around releasing data to the public and major maintenance to the telescope system during full survey operations, and it's these major milestones that are tracked and reported on in the annual \gls{NOIRLab} \gls{POP} and \gls{SLAC} \gls{FWP}.
...
Teams record their day to day work and progress in \gls{JIRA} and it is reported to the Agencies through our managing organizations.
%not sure how much this is really happening and whether it is happening enough to keep it here in this document?
...
Rubin Observatory is a Program of \gls{NSF} \gls{NOIRLab}.
The Rubin Observatory \gls{Director} is Robert Blum and the Rubin Deputy \gls{Director} for \gls{SLAC} is Phil Marshall.
...
They assist in developing the annual \gls{POP}, tracking milestones and reporting on progress.

On the \gls{SLAC} side, Christine Soldahl is the \gls{Business Manager} who handles similar tasks.

\gls{Rubin Operations} has two operational Departments and one Program in addition to the Director’s Office: \gls{RDO}), \gls{RSO}, \gls{RDM}, and \gls{REO}, a program within Rubin). Each operational Department is led by an \gls{AD}; REO is led by a Head of Program).

\subsection{Responsibility and Authority}
The \glspl{AD}s and Head of \gls{REO} are given broad authority to organize, manage, and staff their areas within an overall budget and operations plan \citedsp{RDO-018} managed by the \gls{RDO}. Annual budgets and plans are developed through a scrub process led by the Program Operations team of the \gls{RDO}. This process informs \gls{NSF} and \gls{DOE} side spending and work plans for the upcoming fiscal year. Budgets and plans are captured in the NOIRLab \gls{POP} and the \gls{SLAC} \gls{FWP} submitted to \gls{NSF} and \gls{DOE}, respectively.

The \glspl{AD} and Head of \gls{REO} are delegated the responsibility for detailed staffing plan, activities, and major milestones to be delivered in their respective areas as well as other aspects of the plan as assigned by the \gls{RDO}. Staffing roles that are vacant are the direct responsibility of the \glspl{AD} and Head of \gls{REO} (or their delegates). Changes to existing roles or promotions/new roles must be approved by the Director or their delegate (usually the Deputy Director). Recruitments are to be initiated by the \glspl{AD} and Head of \gls{REO} (or their delegate) and coordinated with the Program Operations team. Program Operations will work with NOIRLab or SLAC to complete the recruitment once approved by the Director or Deputy Director.

Spending decisions are ultimately held with the Director. Spending authority at \gls{SLAC} is held by the Deputy Director. Each \gls{AD} at \gls{NOIRLab} and the Head of \gls{RDO} have budget authority up to a Lab specified amount in order to approve requisitions and purchases. Spending authority to team leads both at SLAC and NOIRLab are at the discretion of the Deputy Director and the \glspl{AD} and Head of REO. Spending levels will be at the respective lab specified level. \glspl{AD}, the Head of REO, and team leads should approve all purchases and requisitions (including travel) within their limit according to the current fiscal year plan. Unplanned purchases and requisitions less than 10\% of their budgets can be made without approval from the \gls{RDO} as long as there are funds available. Team leads not employed at \gls{SLAC} or \gls{NOIRLab} will be able to make staffing and budgeting decisions as delegated to them by their \gls{AD} or the Head of \gls{REO} but may need a surrogate to process and approve actual requisitions.
...
\gls{Rubin Operations} considers the \gls{POP} to be a Rubin activity, which informs both \gls{NOIRLab} and \gls{SLAC} leadership of the annual Rubin activity including milestones and budget.
...
For \gls{SLAC}, the \gls{POP} informs \gls{SLAC}'s annual planning, which culminates in a \gls{FWP} (FWP) for all \gls{SLAC} High Energy Physics activity including Rubin.
...
Because the \gls{POP} for \gls{NSF} lags the \gls{FWP} for \gls{DOE} by one year, and their submission dates differ by several months, Rubin does high level planning in early \gls{Q2} of the fiscal year (January and February) and plans 2 years ahead, as required for the \gls{SLAC} \gls{FWP}.
...
\tabref{tab:wbs} shows the level 2 and level 3 elements of the \gls{Rubin Operations} work breakdown structure.

\begin{longtable}[]{@{}lll@{}}
\caption{WBS elements for Rubin Observatory} \label{tab:wbs}\\
...
1 & & Rubin \gls{Director}'s Office  \tabularnewline
  ...
  & 1.5 & \hspace{0.5cm} Legacy Survey of Space and Time  \tabularnewline
  & 1.6 & \hspace{0.5cm} Change and Risk Management  \tabularnewline
  & 1.7 & \hspace{0.5cm} Special Investigations  \tabularnewline
2 & & Rubin Summit \gls{Operations}  \tabularnewline
  & 2.1 & \hspace{0.5cm} \gls{RSO} Management  \tabularnewline
  & 2.2 & \hspace{0.5cm} Summit Science  \tabularnewline
  & 2.3 & \hspace{0.5cm} Summit Software  \tabularnewline
  & 2.4 & \hspace{0.5cm} Summit Facility  \tabularnewline
  & 2.5 & \hspace{0.5cm} Nighttime \gls{Operations}  \tabularnewline
  & 2.6 & \hspace{0.5cm} Summit Engineering  \tabularnewline
  & 2.7 & \hspace{0.5cm} Camera  \tabularnewline
  & 2.8 & \hspace{0.5cm} Summit Systems Engineering  \tabularnewline
3 & & Rubin Data Management \tabularnewline
  & 3.1 & \hspace{0.5cm} \gls{RDM} Management  \tabularnewline
  & 3.2 & \hspace{0.5cm} Data Facilities  \tabularnewline
  & 3.5 & \hspace{0.5cm} Summit DevOps  \tabularnewline
  & 3.6 & \hspace{0.5cm} Pipeline Middleware  \tabularnewline
  & 3.7 & \hspace{0.5cm} Data Engineering  \tabularnewline
  & 3.8 & \hspace{0.5cm} Algorithms and Pipelines  \tabularnewline
  & 3.9 & \hspace{0.5cm} Campaign Management  \tabularnewline
  & 3.10 & \hspace{0.5cm} Service Quality and Reliability Engineering  \tabularnewline
  & 3.11 & \hspace{0.5cm} QServ  \tabularnewline
  & 3.12 & \hspace{0.5cm} Build Engineering  \tabularnewline
  & 3.13 & \hspace{0.5cm} Community Science  \tabularnewline
  & 3.14 & \hspace{0.5cm} Data Validation  \tabularnewline
5 & & \gls{REO} \tabularnewline
  & 5.1 & \hspace{0.5cm} Education and Public Outreach  \tabularnewline
...
Team leads will work with their department's \glspl{AD} to develop plans for activities that address specific milestones, projects, and level of effort activity.
Progress towards the highest level milestones is reported regularly throughout the fiscal year to \gls{SLAC}, \gls{AURA}, \gls{NOIRLab}, \gls{NSF} and \gls{DOE}.
...
Progress is tracked at the milestone level by a combination of computing the fraction of time that has passed coupled with progress input from the \gls{AD}.
...
Additionally in certain cases such as operations and maintenance of telescope and facility systems, pipelines or other systems, \gls{LOE} is perfectly acceptable.
...
\gls{Rubin Operations} uses 1,726 hours per year as a fiducial value for effort estimation purposes. This figure is \emph{after} all vacations, sick leave, etc are taken into account. \gls{SLAC} may have different definitions for tracking their staff time. This value is used to estimate effort for activities and deliverables at the department level through planing epics and determines FTE needs.
...
Staff in ``scientist'' roles spend either 20\% (259 hours) or 50\% (777 hours) of their time on personal research (see the \citeds{RDO-018} Plan for details).
...
% Removed DEI time
Personal research time is charged to a \gls{NOIRLab}'s \gls{RSS} project code and is prorated for staff who are fractionally allocated to \gls{Rubin Operations}.
Training, administrative, and outreach time is charged to either \gls{RSS}, \gls{ES}, \gls{DMS}, \gls{CEE} depending on where staff are allocated into \gls{NOIRLab}.
Functional Managers together with Program Operations will ensure the proper project codes are available on timecards.
...
%removing the below table as it is not relevant with the LOE model
...
\item March
   ...
   \end{itemize}
\item April-May: \gls{NOIRLab} \gls{POP} process and \gls{SLAC} \gls{FWP} development kick off mid-April, with major milestones and budget due end of May.
   \begin{itemize}
      \item Early April: set next \gls{FY} major milestones
         \begin{itemize}
            \item \glspl{AD} review and/or add/edit major milestones
            \item Discuss in Ops Exec
            \item Implement changes in \gls{JIRA}
         \end{itemize}
      \item Late April: set next \gls{FY} (and \gls{FY}+2) budget:
         \begin{itemize}
            \item Early April: freeze planning tools (\gls{WBS} and Cost Calculator)
            \item \gls{JOR} - timing can vary
      ...
      \item May: next \gls{FY} Sandboxing:
         \begin{itemize}
            ...
            \item Early May Department management teams facilitate team-level sandbox pitches (including L2/L3 milestones, changes to spending plans)
            \item Late May: Sandboxing Workshop, for all ops teams and \gls{Director}'s Office.
         \end{itemize}
   \end{itemize}
\item June-July: implement sandboxed changes.
   \begin{itemize}
      \item Implement proposed changes to Staffing Plan, \gls{Risk} Register, \gls{JIRA} L2/L3 milestones
      ...
      \item July: derive next \gls{FY} contract and \gls{MPO} \glspl{SOW}.
   \end{itemize}
\item September
   \begin{itemize}
      \item Update lab-wide budget models (\gls{RAS}) from Staffing and Nonlabor plans, for quarterly forecasting.
      ...
      \begin{itemize}
         \item Collect requests, study feasibility
         \item Discuss at Ops-Exec, import changes into \gls{JIRA}
      \end{itemize}
   \end{itemize}
\end{itemize}
...
\begin{enumerate}
    \item A set of \emph{milestones}, each of which represents the delivery of a major aspect of \gls{Rubin Operations}, availability of specific functionality, or maintenance event for the telescope system.
        Milestones are planned in the scrub workbook and then officially defined in a \gls{JIRA} milestone issue.
    \item A series of \emph{epics} describe major pieces of work.
        Epics are associated with concrete, albeit high-level, deliverables or outcomes that culminate in the achievement of the above milestones, and have specific staff assignments and end dates.
        All epics are linked to the milestone they are created to help deliver.
    \item A visualization of progress toward achieving milestones is captured based on time-phased \gls{LOE}, with progress verified and, where necessary, adjusted by the Team Lead. Progress is directly tracked by rolling up the completion of issues within \gls{JIRA} epics, with the linked issues collectively contributing to the delivery of a given milestone.
\end{enumerate}
...
Level 1 and 2 milestones are under change control once they are defined and described in a \gls{JIRA} Milestone issue. Any change there after must follow the defined Change Control process.

Level 3 milestones are defined for use within Departments and not required to go under project change control, but properly adhering to the plan is important: other teams will use these milestones to align their schedules, so they rely on accuracy.
...
Progress is tracked against milestones in issues linked to \gls{JIRA} epics. For activities with defined start and end dates, progress is initially estimated based on elapsed time—for example, an activity scheduled over six months would be assumed to be 50% complete after three months. This time-based progress is then verified by the Team Lead and may be adjusted to reflect the actual status of the activity.

To ensure success, \gls{JIRA} epics must be completely detailed out prior to a full 6-month \gls{cycle} and start and due dates must be estimated.
...
Rubin Milestones are defined as \gls{JIRA} issues of type ``milestone''. As indicated above, the \gls{Director} (or their designate) defines the L1 milestones, the \glspl{AD} define their departments' L2 milestones, and the Team Leaders and \glspl{AD} define the L3 milestones for their teams.

L1, L2 and most L3 milestones are defined as part of the annual planning \gls{cycle}, and prior to the year in which the work associated with them is due to be carried out. \glspl{AD} and Team Leaders specify their milestones in the annual scrub from where the Program Coordinator takes the information and then creates a \gls{JIRA} issue of type ``milestone'' for each one.

During the year, it is sometimes necessary to create new milestones (primarily at Level 3) that were omitted during the earlier planning phase. In this case, the team leader or \gls{AD} may create the \gls{JIRA} milestone directly, and alert the Program Coordinator to it for tracking purposes.
...
\item The \texttt{Milestone Activity} field is equivalent to the activity that will take place in order to produce the deliverable and meet the milestone.  It should contain a sentence outlining the activity to be performed in order for the milestone to be reached and the deliverable to be produced. In the \gls{POP} document tables this is the ``Activity.''
  Examples include \textit{``Deliver Data Preview 0.1 (DP0.1)''} (an L1 milestone) and \textit{``DP0.1 \gls{Data Release}: science-ready catalogs released from the \gls{IDF}''} (an L2 milestone that belongs to it).
...
\item The \texttt{Description} text should contain more information detailing the scope of activity needed to complete the milestone.
  Example: \textit{``Upgrade the \gls{WBS} activity, labor and non-labor plans from V4 to V5 in order to capture a \gls{US} \gls{DF} at SLAC, a \gls{UK} \gls{DF}, and any other modifications needed, and estimate the corresponding budget.''}
...
\item The \texttt{Due Date} is the latest date in the future by which the milestone needs to be reached.
  This date should be before or the same as the milestone's parent milestone's ``Due Date'' of the parent milestone in \gls{JIRA}.
...
The Program Coordinators will ensure that the milestones that have been defined are correctly arranged so that their epics appear nested beneath them.
...
Before the start of a \gls{cycle}, milestones are confirmed by the \gls{Director}'s Office, and detailed in the Milestone Jira issue. Any team member can find the milestones in \gls{JIRA}.
...
In general, this should flow directly from the long term plan (\secref{sec:long-term-plan}), ensuring that the expected planning packages are being worked on and milestones being hit.

While developing the \gls{cycle}:
...
\item Reserve time for planning the following \gls{cycle}: it will have to be defined before the current \gls{cycle} is complete;
...
Ensure that the program of work being planned is achievable by the assigned team in the time available also considering the skills and availability of the team.
...
\begin{itemize}
\item A concrete, well defined deliverable \emph{or} be clearly described as a ``bucket'' or ``emergent work'' (\secref{sec:bucket});
\item The \texttt{Component} field set to the appropriate Department;
\item In the \texttt{Linked Issues} field, create a \textit{``blocks''} link to the parent milestone to reflect how that milestone contributes to achieving that milestone.
\item The \texttt{Start Date} field set to the appropriate date, which is not earlier than the start date of the milestone it is labeled to achieve.
...
\end{itemize}
The fields above are required to have values entered because they define the tracking of progress. Other fields in the \gls{epic} can also be filled in as needed.

Be aware that:

\begin{itemize}
\item An \gls{epic} may only be assigned to a single \gls{cycle}.
  It is not possible to define an \gls{epic} that crosses the \gls{cycle} boundary (see \secref{sec:cycle-close} for the procedure when an \gls{epic} is not complete by the end of the \gls{cycle}).
...
\item The \gls{epic} should be at an appropriate level of granularity.
  While short duration \glspl{epic} may be suitable for some activities, in general \glspl{epic} will describe a few months of time.
\end{itemize}

Although it is possible---indeed, encouraged---to set the \texttt{assignee} field in \gls{JIRA} to the individual who is expected to carry out the bulk of the work or is responsible for the work in an \gls{epic}, this does not provide sufficient granularity for those cases when more than one person will be contributing.
...
As discussed in \secref{sec:long-term-research}, research is sometimes required
...
We therefore schedule research in \gls{timebox}ed \glspl{epic}: we allocate a certain amount of time based on the resources available.
...
(This does not apply to bucket \glspl{epic} (\secref{sec:bucket}), which are, by their nature, \gls{timebox}ed within the \gls{cycle}).

Be aware that if a planned \gls{epic} is not closed it may impact the completion of the milestone it contributes to.  Epics related to milestones must be completed in order for the milestone to also be completed.


% ======================================================================
\section{Execution} \label{sec:execution}

Having defined the plan for a \gls{cycle} following \secref{sec:cycle-plan}, we execute it by means of a series of month-long sprints.
...
Epics have already been defined as part of the \gls{cycle} plan (see \secref{sec:planning-epics}).
...
A \gls{story} describes a planned activity with duration of a few days.
It must be possible to schedule a \gls{story} within a single sprint, so no \gls{story} should ever be allocated more than the number of work days in the month in which it is planned.
...
However, do break \glspl{epic} down to describe the stories which will be worked in an upcoming sprint (\secref{sec:sprinting}) before the sprint starts.
When doing so, you may wish to leave some spare time to handle emergent work (discussed in \secref{sec:bucket}).
Every \gls{epic} should contain \emph{at least} some time for this.

After execution is complete, comparing the orignal planned duration to the actual duration affords the opportunity to refine  estimates going forward.
...
As discussed in \secref{sec:jira-maintenance}, technical managers should regularly monitor \gls{JIRA} for relevant tickets and ensure they are handled appropriately.
...
\item Preparation.
  ...
  The team assigns the work that will be addressed during the sprint by choosing from the pre-defined stories (\secref{sec:defining-stories}).
  Each team member should be assigned a plausible amount of work, based on the \gls{story}.
...
\item Execution.
  ...
  practice includes holding regular ``standup'' meetings (see \secref{sec:sup}), at which
  ...
  properly track progress as per \secref{sec:track-progress}.

  When completing a \gls{story} we may have changed the due date if it was taking longer than expected. 
  In the history of the ticket one can see such changes.
...
\item (If applicable, i.e. it is not a bucket, as defined in \secref{sec:bucket}) its concrete deliverable has been achieved.
\end{enumerate}
...
%Both RFCs mentioned above a very old from 2016 - i'm not sure they are still current with out set up?
This indicates which manager is responsible for seeing that the work is completed successfully.
Available teams, and the associated managers, are listed in the \href{https://developer.lsst.io/processes/jira_agile.html}{Developer Guide}; generally speaking, they align with the the work breakdown structure described in \secref{sec:wbs}.
Where there is uncertainty about which team should be responsible for a particular ticket, the “Data Management” team may be used to indicate that the \gls{AD} of Data Management is responsible for assigning the work.

\gls{JIRA} should be regularly monitored for incomplete, overdue or unassigned tickets and updated appropriately.
Where tickets describe bugs or other urgent emergent work which cannot be deferred, refer to \secref{sec:bucket}.
...
Progress is tracked against milestones in issues linked to JIRA epics. For activities with defined
start and end dates, progress is estimated based on elapsed time

\subsection{Reporting Cycle}

High level milestone progress will be reported to \gls{SLAC} and \gls{NOIRLab} monthly.
\gls{NOIRLab} reports will flow quarterly (or monthly) to the \gls{NSF}.
Rubin will show progress on all L1 milestones and any L2 milestones called out in the \gls{POP}.

% ======================================================================
\section{Annual Budget Planning}
\label{sec:annual-cycle}

Rubin Operations are funded at approximately equal levels by the \gls{NSF} and \gls{DOE}. \gls{NSF} funding is requested by \gls{NOIRLab} through its 5-year renewal proposal process, and annual budgets submitted as part of the \gls{NOIRLab} Program Operations Plan (\gls{POP}). \gls{DOE} funding is requested by \gls{SLAC} in an annual Field Work Proposal (FWP). In this section we describe the annual cycle of budget development and spending planning.
...
The major milestones and budget are inserted into the draft \gls{NOIRLab} \gls{POP} and \gls{SLAC} \gls{FWP}.
...
We also invite them to scrub their part of the risk register, update their risk response plans in concert with their planned activities, and anticipate any \glspl{RBT} they may need to make.
...
(In practice, only the three technical departments' management teams are likely to sit through the entire workshop, although it is organized inclusively so that anyone could choose to review all the scrubbed plans.)
...
Following the Sandbox Workshop, the \gls{Director}'s Office approves, adjusts and finalizes the overall spending plan by the end of June, in time for the submission of the NOIRLab \gls{POP}, and \gls{SLAC}'s \gls{DOE} budget briefing.
...
Statements of work for the next fiscal year's contracts and \glspl{MPO} are straightforwardly derived from the scrubbed plans, for their July deadlines.

In September, the teams then plan work (for the first half of the new fiscal year) against their minor milestones, incorporating any community input they may have collected at the annual Community Workshop at the end of July.
...
Additional spending to address realized risks is enabled by Rubin's \gls{RBT} process, whereby reserve funds are released for use by the departments in response to a specific mid-year request made in a custom Jira ticket.
...
\gls{DOE} allows a limited reserve to be built up and carried forward at \gls{SLAC}, and used as contingency reserve for the specific risk of delayed or reduced funding (e.g. resulting from a government shutdown).
\gls{DOE} \gls{HENP} holds additional reserves centrally, which can also be used as both contingency and management reserve.
\gls{NSF} \gls{AST} allows line item budget over-estimation by up to 10\%, to allow for increases in cost.
%do we need to mention NSF Office of Research Infrastructure (ORI)
These generate carry-forward (along with any other under-spending that may occur), which we use as contingency reserve.
\gls{NSF} holds management reserve centrally, to handle unexpected events.
...
(At \gls{SLAC}, for example, this would cover about 7\% of the operating expense -- enough to pay the labor bills during a 1 month funding allocation delay.) The top risks (high probability and at least moderate severity) need to be reported regularly to the funding agencies so that they can plan for the possibility of providing supplemental funds to address these risks if they are realized.
...
(At \gls{NOIRLab}, carry-forward use must be declared a year in advance in the \gls{POP}: this is why the sandboxing process includes a request that the operations teams pitch \glspl{RBT} for the coming year at that stage.)
...
\gls{Review} is by the Directorate team, as budget holders.
...
\end{itemize}
and direct new recruits to them when they join your team\footnote{As per \secref{sec:newcomers}, remember that newcomers should be allocated a Story for working through this material.}.

The responsible must also complete an \href{https://project.lsst.org/onboarding/form}{onboarding form} for the new recruit.
When team members leave the project, the team leader/supervisor should fill in an \href{https://project.lsst.org/onboarding/offboarding_form}{offboarding form}.
