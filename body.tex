...
This document provides a guide to the \gls{VRO}  approach to project management.
...
She monitors the budgets and maintains details within WeBud, the NOIRLab budgeting system. She assists in developing the Program Operating Plan (POP), tracking milestones and reporting. On the SLAC side, Christine Soldhal, Business Manager, handles similar tasks.

\section{Rubin Operations Depeartments and Associate Directors}
\label{sec:tcam}
...
The \gls{WBS} provides a hierarchical description of the activiry based organization of Rubin. In operations, the WBD is not as formal as in a construction project, but it provides a useful structure to prgram annual work around. Rubin if the level one of the WBS, the departments are level 2, and teams within the departments are level 3. Individual roles on operations are level 4.  
...
Work is planned in advance of each financial year at the team level. Team leads will work with department heads to develop plans for activities that address specific milestones, projects, and level of effort activity. 

High level plans are presented to DOE in the first quarter of the calendar year and to NOIRLab somewhat later as part of the POP process. Rubin needs to be ready then early in the calendr year with plans for the comming year. Detailed work plans are developed through the northern summer.
