...
This framework allows the multidisciplinary Rubin team to operate the facility and generate nightly data products while continuously improving efficiency of workflows ('kanban' style\footnote{Kanban is a visual system for managing work as it moves through a process. Kanban is a concept related to lean and just-in-time (JIT) production, where it is used as a scheduling system that tells you what to produce, when to produce it, and how much to produce.}), as well as iteratively responding to user feedback on a longer timescale to maximize the scientific benefit of annual data releases ('scrum' style\footnote{https:\/\/en.wikipedia.org\/wiki\/Scrum\_(software\_development)}).  
...
\section{Rubin Program Management}
\label{sec:contacts}

The Rubin Observatory Acting \ \gls{Director}  is Robert Blum and the Interim Deputy \gls{Director} for NOIRLab is Amanda Bauer and the Deputy Director for SLAC is Phil Marshall.
...
She monitors the budgets and maintains details within WeBud, the NOIRLab budgeting system. She assists in developing the annual Program Operating Plan (POP), tracking milestones and reporting. On the SLAC side, Christine Soldhal, Business Manager, handles similar tasks. The POP is a defined report and process for NOIRLab. Rubin Operations considers the POP to be a Rubin activity, which is informs both NOIRLab and SLAC leadership of the annual Rubin activity including milestones and budget. For NOIRLab, the Rubin POP is integrated and delivered to NSF for the next fiscal year at the end of the current fiscal year. For SLAC, the POP informs SLACs annual planning which culminates in a Field Work Proposal (FWP) for all SLAC High Energy Physics activity including Rubin. The FWP is delivered in June of the current fiscal year for the next fiscal year. The FWP is previewed to SLAC management in February in advance of the final delivery in June. Because the POP for NSF and FWP for DOE are out of phase, Rubin does high level planning in early Q2 of the financial year. Detailed activity planning including lower level milestones continues though the year in advance of the next year. This detailed planning is the subject of this document.

\section{Rubin Operations Departments and Associate Directors}
\label{sec:tcam}

Rubin Operations has four operational Departments and the Director’s Office: RDO (Rubin Director’s Office), ROO (Rubin Observatory Operations), RDP (Rubin Data Production), RPF (Rubin System Performance), and REO (Rubin Education and Public Outreach). Each operational Department is lead by an Associate Director.
...
Detailed work is planned in advance of each financial year at the team level. Team leads will work with department heads to develop plans for activities that address specific milestones, projects, and level of effort activity. Highest level milestones are reported regularly thought the fiscal year to SLAC, AURA, NSF and DOE. 
...
Our first-order estimate is that developers will spent 30\% of their time on \gls{LOE} type activities, and the remaining 70\% of their effort is tracked against concrete deliverables. Technical staff in Chile at the summit facility may spend a much more significant fraction of time as  \gls{LOE}.
However, as above, we generally aspire to minimize the fraction of \gls{LOE} for development activity,
% This is true for development work. We need to prioritize reliable operations. : team leads are therefore encouraged to explicitly schedule as much developer time as is possible.
...
%is \emph{not} chargeable to \gls{LSST} under any \gls{WBS} or
%account, including level of effort. 
Rubin expects to pay the full
rate for an individual with research time who contributes 1,440 hours to
operations. This is handled through indirect rates at both NOIRLab and direct charges to research accounts at SLAC. Science time is included in the sub contracts of our partners at affiliated institutions through indirect charges similar to the case for NOIRLab.
%, and does not require any accounting of the remaining 360 hours.
...
The authoritative summary of the long-term planning system may be found in (find a POP process document).
...
\begin{description}
%this is not consistent with NOIRLab or our current FY21 plan. We need to discuss an decide how to capture these. Could be one or two RDO level MS and several L2 MS for each department both of which show up in the POP. Or we keep the Major MS classification and have several of those in each Dep. Suppose we had one or two L1 milestones like Annual DR. Then all the current Major MS in our sheet could be L2s that point at that.
\item[Level 1]
...
\label{sec:reporting-cycle}
 ...
 High level milestone progress will be reported to SLAC and NOIRLab regularly. NOIRLab reports will flow quarterly to the NSF as ell showinf progress on all L1 and L2 milestones called out in the POP. 
